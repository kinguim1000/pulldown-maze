...
def abs(a):
    ...
    return a
def mapa(input):
    return (800/(1+(60/(abs(input)+0.00001))))*sign(input)
...
def turnLeft(angulo):
    ...
    while(abs(posicao(initial[1])-initial[0])<40):
        irEsquerda(600)
    print(d())
    while(d() > 40):
        irEsquerda(int(mapa(a.out(d()))/2))
        print(d(),posicao(initial[1]))
    parar()
    print(d())

def turnRight(angulo):
    a = PID(0.5,0,5) 
    initial = encoder()
    k = {0:0,1:0,2:0,3:0}
    a.atualizar(posicao(initial[1])-initial[0])
